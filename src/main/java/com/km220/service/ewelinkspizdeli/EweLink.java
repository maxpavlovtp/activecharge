package com.km220.service.ewelinkspizdeli;


import com.km220.service.ewelinkspizdeli.errors.DeviceOfflineError;
import com.km220.service.ewelinkspizdeli.model.Status;
import com.km220.service.ewelinkspizdeli.model.StatusChange;
import com.km220.service.ewelinkspizdeli.model.devices.DeviceItem;
import com.km220.service.ewelinkspizdeli.model.login.LoginRequest;
import com.km220.service.ewelinkspizdeli.model.login.LoginResponse;
import com.km220.service.ewelinkspizdeli.wss.WssLogin;
import com.km220.service.ewelinkspizdeli.wss.WssResponse;
...
public class EweLink {
  ...
  public void getWebSocket(WssResponse wssResponse) throws Exception {
    ...
    eweLinkWebSocketClient = new EweLinkWebSocketClient(
        new URI("wss://" + region + "-pconnect3.coolkit.cc:8080/api/ws"));
    clientWssResponse = wssResponse;
    eweLinkWebSocketClient.setWssResponse(clientWssResponse);
    eweLinkWebSocketClient
        .setWssLogin(gson.toJson(new WssLogin(accessToken, apiKey, APP_ID, Util.getNonce())));
  ...
  public String getDevices() throws Exception {
    ...
    URL url = new URL(
        baseUrl + "/user/device?lang=en&appid=" + APP_ID + "&ts=" + new Date().getTime()
            + "&version=8&getTags=1");
  ...
  private static String getAuthMac(String data)
        ...
        try {
          ...
          try {
            eweLinkWebSocketClient = new EweLinkWebSocketClient(
                new URI("wss://" + region + "-pconnect3.coolkit.cc:8080/api/ws"));
            eweLinkWebSocketClient.setWssResponse(clientWssResponse);
            eweLinkWebSocketClient.setWssLogin(
                gson.toJson(new WssLogin(accessToken, apiKey, APP_ID, Util.getNonce())));
            eweLinkWebSocketClient.connect();
